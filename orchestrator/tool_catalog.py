...
# ============================================================================
# Trip mission — extra tool definitions + per-phase filter.
#
# `BUNQ_TOOLS` above is the full union used by Weekend / Travel / Payday /
# Council. Trip is interactive (multi-turn chat), so we expose a smaller,
# phase-filtered subset on each turn — the model literally can't see the
# execution tools during UNDERSTAND, can't see anything but
# request_confirmation during AWAITING_CONFIRMATION, etc.
# ============================================================================

TRIP_EXTRA_TOOLS: list[dict[str, Any]] = [
    {
...
# Phase tool filters — what the model sees on each turn of the trip mission.
def trip_tools_for_phase(phase: str) -> list[dict[str, Any]]:
    ...
    """
    by_name = {t["name"]: t for t in TRIP_EXTRA_TOOLS}
    by_name.update({t["name"]: t for t in BUNQ_TOOLS})
